...
def do_union(exp_t):
    ...
    start.next_state['ε'] = [first_nfa[0], second_nfa[0]]
    first_nfa[1].next_state['ε'] = [end]
    second_nfa[1].next_state['ε'] = [end]
...
def do_kleene_star(exp_t):
    ...
    start.next_state['ε'] = [starred_nfa[0], end]
    starred_nfa[1].next_state['ε'] = [starred_nfa[0], end]
...
#         if val[0] == st and val[1] == "ε" and val[2] not in nfa["final_states"]:
...
if __name__ == "__main__":
    # print()
    print("UNION : + \t KLEENE: *")
    reg = input("Enter your regex expression : ")
    pr = polish_regex(reg)
    print(pr)
    et = make_exp_tree(pr)
    fa = compute_regex(et)
